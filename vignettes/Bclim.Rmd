...
plot(Roya_histories$slice_clouds,
     slice=50, 
     dims=2:3, 
     main='slice 50',
     xlab='MTCO',
...
par(mar=c(3,3,2,1), mgp=c(2,.7,0), tck=-.01,las=1)
plot(Roya_histories, 
     most_representative = 3, 
     chron=Roya_chronologies,
     xlab='Age (k cal years BP)',
     ylab='GDD5',
     main='Roya GDD5')
legend('topleft',
       legend=c('Slice clouds',
                'Climate ribbon',
...
Running the same as above but with the Glendalough data
...
# load in dates
Glendalough_dates = read.table(system.file("extdata", 
                                    "Glendalough.dat", 
                                    package = "Bclim"), 
                        header=TRUE)

# Load in the pollen depths
Glendalough_depths = read.table(system.file("extdata", 
                                            "Glendalough_pollen_depths.txt",
                                            package = "Bclim"))

# Run Bchron
Glendalough_chron = with(Glendalough_dates,
                  Bchronology(ages=Age, 
                              ageSds=Error, 
                              calCurves = c('normal',rep('intcal13',5)),
                              positions=Depth, 
                              positionThickness=Thickness, 
                              id=id, 
                              predictPositions=Glendalough_depths$V1,
                              thin = 4))
```
...
```{r, results='hide'}
# Load in the pollen data
Glendalough_pollen = read.table(system.file("extdata",
                                    "Glendalough_pollen.txt",
                                    package = "Bclim"),
                        header=TRUE)

# Extract the chronologies - Bclim requires them in thousands of years
Glendalough_chronologies = Glendalough_chron$thetaPredict/1000

# Run the single slice climate clouds
Glendalough_slices = slice_clouds(Glendalough_pollen)

# Run the climate histories function
Glendalough_histories = climate_histories(slice_clouds = Glendalough_slices,
                              chronology = Glendalough_chronologies,
                              time_grid = seq(0, 14, by = 0.1))
```
...
# Plot a single slice cloud if required - plot slice 50
par(mar=c(3,3,2,1), mgp=c(2,.7,0), tck=-.01,las=1)
plot(Glendalough_histories$slice_clouds,
     slice=50, 
     dims=2:3, 
     main='slice 50',
     xlab='MTCO',
...
# Plot the climate histories
par(mar=c(3,3,2,1), mgp=c(2,.7,0), tck=-.01,las=1)
plot(Glendalough_histories, 
     most_representative = 3, 
     chron=Glendalough_chronologies,
     xlab='Age (k cal years BP)',
     ylab='GDD5',
     main='Glendalough GDD5')
legend('topleft',
       legend=c('Slice clouds',
                'Climate ribbon',
                'Representative histories'),
...
I haven't created any further plots here, but they are available by simply copying the rest of the Roya code above.


```{r, include = FALSE, eval = FALSE}
cairo_ps(file = 'Glendalough_MTCO_20170428.eps', width = 12, height = 6)
#pdf(file = 'Glendalough_MTCO_20170428.pdf', width = 12, height = 6)
par(mar=c(3,3,2,1), mgp=c(2,.7,0), tck=-.01,las=1)
plot(Glendalough_histories,
     dim = 2,
     most_representative = 1,
     chron=Glendalough_chronologies,
     conf = c(0.95, 0.5),
     slice_clouds = FALSE,
     ylim = c(-30, 10),
     ...
     main='Glendalough - Mean Temperature of the Coldest Month')
legend('bottomleft',
       legend=c('Climate 50/95% uncertainty ribbon',
